...
def parse_args():
    ...
    parser.add_argument("--prefix_count", default=0, type=int)
...
def infer(args):
    ...
    for data_name in tqdm(data_names):
        examplea=[]
        with open('/project/ugmathllm/caiyuchen/LIMO/eval/auto_temperature/fig/drgrporank1base_aime_results_gpu1.json','r') as f:
            dats = json.load(f)
        prefix_count = args.prefix_count
        for data in dats:
            #reson = data['full_text'] - data['prompt']
            full = tokenizer.encode(data['full_text'], add_special_tokens=False)
            prompt = tokenizer.encode(data['prompt'], add_special_tokens=False)

            aa = tokenizer.decode(full[:len(prompt)+prefix_count])
            examplea.append(aa)

        if args.end_idx == -1:
            args.end_idx = len(examplea)
        examplea = examplea[args.start_idx:args.end_idx]
        ...
        # 准备所有prompts
        prompt_batch = []
        for idx, example in tqdm(enumerate(examplea)):
